...
def paint3d():
    paint3d_results = 'Paint3D/results'
    for mesh_folder in os.listdir(paint3d_results):
        mesh_folder_path = os.path.join(paint3d_results, mesh_folder)
        
        if os.path.isdir(mesh_folder_path):
            for file_name in os.listdir(mesh_folder_path):
                if file_name.endswith('.obj'):
                    obj_file_path = os.path.join(mesh_folder_path, file_name)
                    glb_file_path = os.path.splitext(obj_file_path)[0] + '.glb'
                    save_mesh_as_glb(obj_file_path, glb_file_path)
